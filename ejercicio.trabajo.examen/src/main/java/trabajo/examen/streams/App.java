...
public class App {
	...
	public static void main(String[] args) {
		...
		ejercicioExtraStreams(billetes1);
		ejercicioAllMatch(billetes1);
		ejercicioAnyMatch(billetes1);
	}

	private static void ejercicioAllMatch(List<Billete> billetes) {
		Stream<Billete> billStream = billetes.stream();
		boolean allM = false;

		allM = billStream.allMatch(preciosBilletes -> preciosBilletes.getPrecio() > 25);
		System.out.println("Todos los billetes valen más de 25: " + allM);
	}

	private static void ejercicioAnyMatch(List<Billete> billetes) {
		Stream<Billete> billStream = billetes.stream();
		boolean allM = false;

		allM = billStream.anyMatch(preciosBilletes -> preciosBilletes.getPrecio() > 25);
		System.out.println("Hay billetes por más de 25: " + allM);
	}

	private static void ejercicioExtraStreams(List<Billete> billetes1) {
		// Nombre de los pasajeros que han comprado un billete por menos de 30€
		List<String> pasajeros = billetes1.stream().filter(billete -> billete.getPrecio() < 30)
				.map(Billete::getPasajero).map(Pasajero::getName).collect(Collectors.toList());
	...
	private static void ejercicioStreams2(List<Vuelo> vuelos) {
		...
		List<Vuelo> vueloRes = vueloStream.filter(vuelo -> vuelo.getPlazas() > 100)
				.filter(a -> a.getFecha().isAfter(LocalDate.now())).collect(Collectors.toList());
